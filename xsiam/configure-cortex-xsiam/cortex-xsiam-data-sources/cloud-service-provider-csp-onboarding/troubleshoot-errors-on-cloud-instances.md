...
6. **Applying Terraform updates:** When an **Update available** notification appears for your cloud instance, download the updated Terraform template and run it from your original local deployment folder. Terraform relies on the state file stored in this directory to update existing cloud resources in place instead of creating duplicates. [Learn more about Terraform](introduction-to-terraform-for-cloud-service-provider-csp-onboarding).
7.  Further investigate errors by running XQL queries on the `cloud_health_auditing` dataset.
