---
description: Learn how to build XQL queries with Query Builder in Cortex XSIAM.
---
...
* When you have more than one dataset or lookup, you can change your default dataset by navigating to **Settings** → **Configurations** → **Data Management** → **Dataset Management**, right-click on the appropriate dataset, and select **Set as default**. For more information about setting default datasets, see [Dataset management](https://app.gitbook.com/s/FOhYBYLdbwpnbJgr6uaX/cortex-xdr-3.x-documentation/data-management/dataset-management).
{% endhint %}
...
*   Cold Storage queries use the format `cold_dataset = <dataset name>`.
    ...
    <div data-gb-custom-block data-tag="hint" data-style="info" class="hint hint-info"><p>You can build a query that investigates data in both a cold dataset and a hot dataset in the same query. In addition, as the hot storage dataset format is the default option and represents the fully searchable storage, this format is used throughout this guide for investigation and threat hunting. For more information on hot and cold storage, see <a href="../../../configure-cortex-xsiam/data-management/dataset-management">Dataset management</a>.</p></div>
...
There is no practical limit to the number of stages that you can specify. See [Stages](https://app.gitbook.com/s/fUtoMSNyY2P8jbK3cQsM/readme/stages) for information on all the supported stages.
...
XQL queries can contain different components, such as functions and stages, depending on the type of query you want to build. For a complete list of the syntax options available with example queries, see [Stages](https://app.gitbook.com/s/fUtoMSNyY2P8jbK3cQsM/readme/stages) and [Functions](https://app.gitbook.com/s/fUtoMSNyY2P8jbK3cQsM/readme/functions).
