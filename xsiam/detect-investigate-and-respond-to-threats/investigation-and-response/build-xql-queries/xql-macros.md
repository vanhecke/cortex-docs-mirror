...
<table><thead><tr><th width="120">Name</th><th width="102">Type</th><th width="103">Required</th><th>Description</th></tr></thead><tbody><tr><td><code>macro_name</code></td><td>string</td><td>Yes</td><td>The name of the macro as saved in the Macro Library. Must be enclosed in double quotes.</td></tr><tr><td><code>param</code></td><td>key=value</td><td>No</td><td>One or more parameters to pass to the macro. Parameters are substituted into the macro definition where <code>${param}</code> placeholders appear. Multiple parameters are separated by commas.</td></tr></tbody></table>
...
<table><thead><tr><th width="192">View</th><th>Display behavior</th></tr></thead><tbody><tr><td>Query Builder Editor</td><td>Hover over a <code>call_macro</code> statement to see the macro definition in an inline overlay. Click to expand and replace the macro call with the literal code (undo supported).</td></tr><tr><td>Query History</td><td>Shows the fully expanded query that was executed (all macros substituted).</td></tr><tr><td>Active Queries</td><td>Shows the original query text with <code>call_macro</code> statements (pre-substitution).</td></tr><tr><td>Scheduled Queries</td><td>Shows the original query text with <code>call_macro</code> statements (pre-substitution).</td></tr></tbody></table>
